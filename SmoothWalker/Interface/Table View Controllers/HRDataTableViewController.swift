...
protocol HRDataTableViewControllerDelegate: HRDataTableViewController {
...
class HRDataTableViewController: DataTableViewController {
    
    // MARK: - View Life Cycle Overrides
    
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        navigationItem.title = "Heart Rate"
    }
...
//
extension HRDataTableViewController: HRDataTableViewControllerDelegate {
    ...
    private func processHealthSample(with value: Double) -> HKObject? {
        let dataTypeIdentifier = "Heart Rate"
        let unit = HKUnit.count().unitDivided(by: HKUnit.minute())
        
        guard
            let sampleType = getSampleType(for: dataTypeIdentifier) //数据类型是HKSampleType
        else {
            return nil
        }
